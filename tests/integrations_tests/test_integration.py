...
def test_user_connect_book_quit(tmp_path, monkeypatch, client):
    ...
    clubs, competitions = loadClubs(), loadCompetitions()
    ...
    with client:
        ...
        assert data.find("Sorry, this email is not registered") == -1, print(club["email"])
        ...
        competition_name = form_element.find("input", attrs={"name":"competition"})["value"]
        # get available places
        places_available_element = soup.find(string=re.compile("Places"))
        places_available = re.search(r"Places available: (\d+)", places_available_element).group(1)
        
        assert club["name"] == club_name

        # check that the club has enough point to buy the places
        if not int(club["points"]) > 0 and int(club["points"]) > 5:
            club["points"] = 5
