...
def test_isCompetitionActive():
    ...
    results = [
        server.isCompetitionActive(compet["date"])
        for compet
        in list_competitions
        ]
...
def test_limit_booking_places(client, monkeypatch):
    ...
    assert result.status_code == 400
    assert data.find(
        "<li>You cannot book more than 12 places per competition</li>"
        ) != -1, print(data)
...
def test_book_statut_code_302(client, monkeypatch):
    ...
    response = client.get('/book/test/crash')
